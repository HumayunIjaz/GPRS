...
class login 
{
	...
	function __construct($database){
		$this->link = mysqli_connect("localhost","root","",$database);
		if(!$this->link){
			echo mysqli_error($this->link);
		}
	}
	
	function query($tab,$arr)
	{
		
		$this->query = "SELECT * FROM ".$tab." WHERE ";
		$x=1;
		...
		while($data = mysqli_fetch_array($result)){
			if($data['username']==$arr['username'] && $data['pass']== $arr['pass']){
...
$obj = new login("portfolio");
$obj->query("users",array("username"=>$username,"pass"=>$pass));
